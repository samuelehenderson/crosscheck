// Modal to send a selected player to another team. Shows a searchable list of
// the other 31 teams; picking one records the trade asset.
...
interface Props {
  ...
  fromTeam: Team
  teams: Team[]
  onConfirm: (toTeamId: string) => void
  onClose: () => void
}

export function TradeModal({ player, fromTeam, teams, onConfirm, onClose }: Props) {
  const [query, setQuery] = useState('')

  const options = useMemo(() => {
    const q = query.trim().toLowerCase()
    return teams
      .filter((t) => t.id !== fromTeam.id)
      .filter((t) => !q || `${t.city} ${t.name} ${t.id}`.toLowerCase().includes(q))
      .sort((a, b) => `${a.city}`.localeCompare(b.city))
  }, [teams, fromTeam.id, query])
      ...
      >
        <div className="flex items-center gap-3 border-b border-rink-700 p-4">
          <div
            ...
            style={{
              background: `linear-gradient(135deg, ${fromTeam.colors.primary}, ${fromTeam.colors.secondary})`,
            }}
          >
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="text-xs text-slate-400">
              {fromTeam.city} {fromTeam.name} · {player.position} · {player.overall} OVR
            </div>
          </div>
          <button
            onClick={onClose}
